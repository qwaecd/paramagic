...
void main(){
    ...
    for (int i=0; i<MAX_RINGS; ++i){
        ...
        mask += ring(p, u_ringR[i], u_ringW[i]) * u_ringI[i];
    }

    // 2) 刻度 + 仅在指定环带上出现（提升层次感）
    float bandMask = smoothstep(0.60, 0.62, r) * smoothstep(0.98, 0.96, r);
    mask += sectorTicks(theta, u_sectors, u_tickWidth) * 0.7 * bandMask;

    // 3) 径向条纹（r 方向等距）
    mask += radialBands(p, u_bandCount, u_bandWidth) * 0.6 * bandMask;

    // 4) 扫掠扇形（绕圈）
    float sweep = fract(theta/(2.0*3.14159265) + u_time * u_sweepSpeed);
    mask += (1.0 - aastep(u_sweepWidth, abs(sweep - 0.5))) * 0.7
    * smoothstep(0.70, 0.80, r) * smoothstep(0.95, 0.88, r);

    // 5) 中心核心
    mask += smoothstep(u_coreInner, 0.0, r) * 0.5;
    ...
    vec3 emissiveComponent = u_emissiveColor * u_emissiveIntensity * mask;
    FragColor = vec4(u_baseColor.rgb * mask + emissiveComponent, u_baseColor.a * mask);
    if (u_emissiveIntensity > 0.0) {
        vec3 emissiveHdrColor = (u_baseColor.rgb + u_emissiveColor * u_emissiveIntensity) * mask;
        float brightness = dot(emissiveHdrColor.rgb, vec3(0.2126, 0.7152, 0.0722));
        float bloomThreshold = 1.0;

        if (brightness > bloomThreshold) {
            BloomColor = vec4(emissiveHdrColor - bloomThreshold, 1.0);
        } else {
